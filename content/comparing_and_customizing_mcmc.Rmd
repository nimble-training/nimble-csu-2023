...
modelUncLen = nimbleModel(DEcode, constants = DEconstants_uncLen,
                          inits = DEinits_vals_uncLen, data = DEdata)
```
...
modelHMC <- nimbleModel(DEcode_hmc, constants = DEconstants,
              inits = DEinits_vals, data = DEdata, buildDerivs = TRUE)
...
That mixes well on a per iteration basis, but notice that it took a lot more time (15 seconds compared to 1-2 second for our previous MCMCs) to get the same number of samples. Later on, we'll properly assess mixing in light of sampling time.
